<script lang="ts">
	...
	let { mode, size = 20 }: Props = $props();

	// Edge configurations based on standard topology notation
	// Vertical edges (left/right): use DOUBLE arrows (two parallel chevrons)
	// Horizontal edges (top/bottom): use SINGLE outlined triangular arrows
	// This distinguishes the two identification directions visually
	...
	// - Same direction arrows = normal wrap (cylinder/torus)
	// - Opposite direction arrows = flip wrap (möbius/klein/projective)
...
>
	<!-- Square base: 5 to 19, center at 12 -->
	<rect x="5" y="5" width="14" height="14" rx="0.5" />
	
	<!-- LEFT EDGE: Double chevron centered at y=12 (10.5 and 13.5) -->
	{#if config.left !== 'none'}
		{@const isUp = config.left === 'up'}
		{#if isUp}
			<!-- Pointing up: tips at 9 and 12, centered around 10.5 and 13.5 -->
			<path d="M3 12 L5 9 L7 12" stroke-width="0.7" />
			<path d="M3 15 L5 12 L7 15" stroke-width="0.7" />
		{:else}
			<!-- Pointing down: tips at 12 and 15, centered around 10.5 and 13.5 -->
			<path d="M3 9 L5 12 L7 9" stroke-width="0.7" />
			<path d="M3 12 L5 15 L7 12" stroke-width="0.7" />
		{/if}
	{/if}
	
	<!-- RIGHT EDGE: Double chevron centered at y=12 -->
	...
	<!-- TOP EDGE: Single chevron centered at x=12 -->
	{#if config.top !== 'none'}
	...
	<!-- BOTTOM EDGE: Single chevron centered at x=12 -->
