<script lang="ts">
	...
	function loadNlcaConfigFromStorage() {
		try {
			...
			// Restrict to square neighborhoods only.
	...
	async function ensureNlcaReady(width: number, height: number) {
		...
		// Initialize or reset agent manager
		if (!nlcaAgentManager) {
		...
		if (!nlcaRunId) {
			...
			console.log(`[NLCA] New run started: ${nlcaRunId}, grid: ${width}x${height}`);
		}
	...
	async function startNlcaStep() {
		...
		(async () => {
			try {
				// Optional: apply continuous seeding before sampling the frame.
				...
				if (metrics) {
					// latency8 is ~10ms buckets. Convert to approximate ms.
					let sum = 0;
					...
					// Push per-cell metrics into the renderer for color modulation.
				...
				// Auto-pause on error to avoid spamming requests.
...
let pendingStrokeBefore: Promise<Uint32Array> | null = null;
	...
	onMount(() => {
		...
		const onNlcaConfigChanged = (e: Event) => {
			...
			if (
				...
				// Keep sim rule aligned too, since it’s embedded in the Simulation instance.
			...
			// Recreate stepper & start a new run for changed config.
			nlcaRunId = '';
			if (simulation) {
				void ensureNlcaReady(simState.gridWidth, simState.gridHeight);
			}

			if (typeof detail.gridWidth === 'number' && typeof detail.gridHeight === 'number') {
				// Resize triggers ensureNlcaReady() too.
				resize(detail.gridWidth, detail.gridHeight);
			}
		};
	...
	async function initializeWebGPU() {
		...
		// NLCA mode uses fixed 64x64 grid by default
		// User can change this via NLCA Settings modal
		const width = simState.gridWidth > 0 ? simState.gridWidth : 64;
		const height = simState.gridHeight > 0 ? simState.gridHeight : 64;
		...
		// Use grid dimensions for initial canvas size
		const dpr = window.devicePixelRatio || 1;
		const initialCanvasWidth = width * 10 * dpr;
		const initialCanvasHeight = height * 10 * dpr;
	...
	function animationLoop(timestamp: number) {
		...
		// Run NLCA steps if playing (async, 1 in-flight at a time)
		if (simState.isPlaying) {
			...
			// We do not “catch up” on missed time — LLM stepping is expensive.
			if (!nlcaStepInFlight && simAccMs >= stepMs) {
				// Prevent runaway accumulation while the request is in-flight.
	...
	export function stepOnce() {
		if (!simulation) return;
		// NLCA stepping is async; we fire a single step when paused.
		startNlcaStep();
	}
	...
	export function setScale(scale: GridScale) {
		...
		const { width, height } = calculateGridDimensions(scale, viewport.width, viewport.height, false);
