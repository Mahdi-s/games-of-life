<script lang="ts">
	...
	function updateTourStyles() {
		...
		const [r, g, b] = simState.aliveColor;
		const currentAccentColor = `rgb(${Math.round(r * 255)}, ${Math.round(g * 255)}, ${Math.round(b * 255)})`;
		tourStyleElement.textContent = getTourStyles(currentAccentColor, simState.isLightTheme);
	}
